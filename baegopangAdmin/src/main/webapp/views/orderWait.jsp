								...
								<ul class="pager">
									<c:choose>
										...
										<c:when test="${pageBean.currentPage <= pageBean.totalPage}">
											<li>
												<a href="orderWait.do?pages=${pageBean.currentPage-1}">Previous</a>
											</li>
										</c:when>
									</c:choose>
										...
										<c:forEach var="i" begin="${pageBean.startPage}" end="${pageBean.endPage}" step="1">
											<c:choose>
												<c:when test="${pageBean.currentPage == i}">
													<li>
								 						<a href="orderWait.do?pages=${i}"><strong>${i}</strong></a>
								 					</li>
												</c:when>
												<c:otherwise>
													<li>
								 						<a href="orderWait.do?pages=${i}">${i}</a>
								 					</li>
												</c:otherwise>
											</c:choose>
										</c:forEach>
									...
									<c:choose>
										...
										<c:when test="${pageBean.currentPage < pageBean.totalPage}">
											<li>
												<a href="orderWait.do?pages=${pageBean.currentPage+1}">Next</a>
											</li>
										</c:when>
									</c:choose>
								</ul>
